<template>
  <div class="table flex-col">
    ...
    <div class="option flex-row" v-for="(r, index) in val">
      <span>{{ r.label }}</span>
      <m-radio style="margin: 0;" v-for="c in option.column" :nativeValue="c" v-model="r.value" @input="handleInput(arguments[0], index)"></m-radio>
      <!--<mu-radio v-for="c in option.column" :nativeValue="c" :name="r.label" v-model="r.value" @input="handleInput(arguments[0])"/>-->
    </div>
  </div>
</template>
