  ...
  <div id="nav">
    <div class="nav flex-row">
      <div class="flex-row left">
        ...
        <router-link to='/home' class="appname">患者登记</router-link>
      </div>
      <div class="flex-row right">
        <i @click="go('notice')" class="iconfont icon-tongzhi">
          <span class="notice" v-if="$root.noticeNum > 0">{{$root.noticeNum}}</span>
        </i>
        <i @click="go('contact')" class="iconfont icon-patient"></i>
      ...
      <popover ref="cloud" class="cloud flex-row" style="align-items: flex-start">
    ...
    methods: {
      ...
      exit () {
        ...
        if (process.env.NODE_ENV === 'development') {
          ...
          window.location.href = window.location.origin + '/login/loginform?redirect=mobile'
        }
      }
    }
  }
...
<style scoped>
  ...
  #nav {
    ...
    top: 0;
    z-index: 99999;
    box-shadow: 0 2px 2px 0 rgba(0,0,0,.14), 0 3px 1px -2px rgba(0,0,0,.03), 0 1px 5px 0 rgba(0,0,0,.12);
  }
  .nav {
    margin: 0 auto;
